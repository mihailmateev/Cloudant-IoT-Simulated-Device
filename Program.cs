...
namespace CloudantCRUD
{
    class Program
    {
            ...
            using (var client = CreateHttpClient(handler, user, database))
            {
                ...
                do
                {
                    while (!Console.KeyAvailable)
                    {
                        ...
                        //var creationSensorResponse = Create(sensor.modified.ToString() , client, sensor);
                        ...
                        var updateSensorResponse = Update(client, _id, new { origtime = sensor.origtime, displacement = sensor.displacement, temp = sensor.temp, hmdt = sensor.hmdt, modified = DateTime.Now.ToJavaScriptMilliseconds(), record = sensor.record, recordtitle = sensor.recordtitle, tags = sensor.tags + " and Updated", _rev = rev1 });
        ...
        private static HttpResponseMessage Create(string id, HttpClient client, object doc)
        {
            var json = JsonConvert.SerializeObject(doc, Formatting.None);
            return client.PostAsync(id, new StringContent(json, Encoding.UTF8, "application/json")).Result;
        }
        #endregion Create
